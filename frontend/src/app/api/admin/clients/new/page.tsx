...
export default function NewClientPage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/admin/clients", {
        ...
        body: JSON.stringify({ ...form, durationMonths: Number(form.durationMonths) }),
      });
      ...
      setCreated({
        clientId: result.organization?.client_id,
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="font-outfit text-2xl font-semibold">Create Client</h1>
        <p className="text-sm text-[#71717A]">Creates the tenant, owner login, limits, expiry, and optional Vapi assistant.</p>
      </div>

      {created && (
        <Alert className="border-[#00F0FF]/30 bg-[#00F0FF]/5">
          <AlertDescription className="text-[#D4D4D8]">
            ✅ Client created! Client ID: <span className="text-[#00F0FF]">{created.clientId}</span>, Email: {created.email}, One-time password: <span className="text-[#00F0FF]">{created.password}</span>
          </AlertDescription>
        </Alert>
      )}
      ...
      <form onSubmit={submit} className="glass-card p-6 space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input type="number" min="1" value={form.durationMonths} onChange={(event) => setForm({ ...form, durationMonths: event.target.value })} />
          </div>
        </div>
        <div className="flex gap-3">
          ...
          <Button type="button" variant="secondary" onClick={() => router.push("/admin/clients")}>Back to Clients</Button>
        </div>
      </form>
    </div>
  );
