...
export default async function ClientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: org }, { data: users }, { data: sessions }, { data: calls }, { data: leads }, { data: appointments }] = await Promise.all([
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#71717A]">{org.client_id} · {org.plan} plan</p>
        </div>
        <Link href="/admin/clients"><Button variant="secondary">Back</Button></Link>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {[
          ["Calls", calls?.length || 0],
          ["Minutes Used", minutes],
          ["Minutes Left", Math.max((org.calls_limit || 0) - (calls?.length || 0), 0)],
          ["Leads", leads?.length || 0],
          ["Appointments", appointments?.length || 0],
        ].map(([label, value]) => (
          <div key={label} className="glass-card p-5">
            <p className="text-xs text-[#71717A]">{label}</p>
            <p className="font-outfit text-2xl font-semibold mt-2">{value}</p>
          </div>
        ))}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass-card p-5 space-y-3">
          ...
          <p className="text-sm text-[#A1A1AA]">Expires: {org.expires_at ? new Date(org.expires_at).toLocaleString() : "No expiry"}</p>
        ...
        <div className="glass-card p-5 space-y-3">
          ...
          {(users || []).map((user) => (
            <div key={user.id} className="border-b border-white/5 pb-3 last:border-0">
              ...
              <p className="text-xs text-[#71717A]">{user.role} · {user.login_count || 0} logins · last {user.last_login ? new Date(user.last_login).toLocaleString() : "never"}</p>
            </div>
          ))}
          {(sessions || []).map((session) => (
            <div key={session.id} className="text-xs text-[#A1A1AA]">
              Active session: {session.ip_address || "unknown IP"} · expires {new Date(session.expires_at).toLocaleString()}
            </div>
          ))}
      ...
      <div className="glass-card p-5">
        ...
        <p className="text-sm text-[#71717A] mt-2">RAG is not enabled for Manolle right now, so no knowledge files are managed here.</p>
      </div>
    </div>
  );
}
